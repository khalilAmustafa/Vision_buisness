...
from ui.theme import apply_theme, load_theme_preference, save_theme_preference, ACCENTS


class ManagerWindow(QMainWindow):
    def __init__(self, db: Database, app: QApplication):
        super().__init__()
        self.db = db
        self.app = app
        ...
        self._report_timer.timeout.connect(self.refresh_reports)
        self._report_timer.start(5000)  # refresh every 5 sec
    ...
    def _build_header_card(self) -> QWidget:
        ...
        # Object names below are used by theme.py to apply more colorful styles
        self.metric_total = self._create_metric_widget("Total Users", object_name="MetricCardTotal")
        self.metric_employees = self._create_metric_widget("Employees", object_name="MetricCardEmployees")
        self.metric_managers = self._create_metric_widget("Managers", object_name="MetricCardManagers")
    ...
    def delete_user(self):
        # Prefer selected row ID; fallback to the ID input box
        selected_row = self.table.currentRow()
        user_id = ""

        if selected_row >= 0:
            item = self.table.item(selected_row, 0)  # ID column
            if item:
                user_id = item.text().strip()

        if not user_id:
            user_id = self.id_input.text().strip()

        if not user_id:
            QMessageBox.warning(self, "Delete User", "Select a user or enter a User ID.")
            return

        if user_id == "0000":
            QMessageBox.warning(self, "Delete User", "You cannot delete the default manager (0000).")
            return

        confirm = QMessageBox.question(
            self,
            "Confirm Delete",
            f"Delete user {user_id} and ALL related logs/summaries/shifts?",
    ...
    def _create_metric_widget(self, label_text: str, object_name: str = "Card") -> QWidget:
        card = QFrame()
        card.setObjectName(object_name)

        # Keep padding here; theme.py can override visuals (gradient/border/etc.)
        card.setStyleSheet("QFrame { padding: 12px; }")
    ...
    def _build_tabs(self):
        ...
        shifts_layout.setSpacing(14)

        shift_table_card = QFrame()
        shift_table_card.setObjectName("Card")
        shift_table_layout = QVBoxLayout(shift_table_card)

        shift_table_layout.addWidget(QLabel("Shift Schedule (HH:mm)"))

        self.shift_table = QTableWidget()
        self.shift_table.setColumnCount(3)
        self.shift_table.setHorizontalHeaderLabels(["User ID", "Start", "End"])
        ...
        shift_table_layout.addWidget(self.shift_table)
...
class LoginWindow(QMainWindow):
    def __init__(self, db: Database, app: QApplication):
        ...
        # -------- Theme controls (Dark/Light + Accent) --------
        ...
        # load current saved
        self._theme_mode, self._theme_accent = load_theme_preference()
        self.accent_combo.setCurrentText(self._theme_accent)
        if self._theme_mode == "light":
            self.theme_toggle.setText("Dark Mode")

        self.theme_toggle.clicked.connect(self._toggle_mode)
        ...
        theme_container.setSpacing(10)

        theme_container.addStretch(1)  # pushes controls to the right
        ...
        theme_container.addWidget(QLabel("Theme"))
        theme_container.addWidget(self.theme_toggle)
        theme_container.addWidget(QLabel("Accent"))
        theme_container.addWidget(self.accent_combo)

        form_layout.addLayout(theme_container)
    ...
    def open_manager(self):
        if self._manager_window is None:
            self._manager_window = ManagerWindow(self.db, self.app)
    ...
    def _toggle_mode(self):
        mode, accent = load_theme_preference()
        new_mode = "light" if mode == "dark" else "dark"
        save_theme_preference(new_mode, accent)
        apply_theme(self.app, mode=new_mode, accent=accent)
        self.theme_toggle.setText("Dark Mode" if new_mode == "light" else "Light Mode")

    def _change_accent(self, accent_name: str):
        mode, _ = load_theme_preference()
        save_theme_preference(mode, accent_name)
        apply_theme(self.app, mode=mode, accent=accent_name)
...
def main():
    ...
    mode, accent = load_theme_preference()
    apply_theme(app, mode=mode, accent=accent)

    window = LoginWindow(db, app)
    window.show()
    sys.exit(app.exec_())
